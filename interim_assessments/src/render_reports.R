...
ge_list <- regional_school_rollup %>% 
  ...
  mutate(subject = str_extract(local_assessment_id, "ELA|Math|Science"), #Add Algebra for IA 2
         grade = as.integer(str_extract(local_assessment_id, "0\\d"))) %>% 
  filter(!is.na(pct_correct),
         !is.na(subject)) %>% #this filters out Algebra - may want to remove for IA 2
...
render_ge_math_sci <- function(school, subject, grade) {
  ...
  rmarkdown::render(here("SY 18-19 IA 03-8 Math Report.Rmd"), params = list(
    school = school,
    subject = subject,
    grade = grade
  ),
...
render_ge_ela <- function(school, subject, grade) {
  ...
  parent_folder_name <- grade_folders[[subject]]
  sub_folder_id <- parent_folder_name[[grade-2]]$id
  rmarkdown::render(here("SY 18-19 IA 03-8 ELA Report.Rmd"), params = list(
    school = school,
    subject = subject,
    grade = grade
  ),
...
render_dl <- function(test_id, test, teacher, school) {
  ...
  sub_folder_id <- if_else(subject == "ELA",
                           dl_ela[[grade-2]]$id,
                           dl_math[[grade-2]]$id)
  rmarkdown::render(here("SY 19-20 DL IA Report.Rmd"), params = list(
    test_id = test_id,
    test = test,
    teacher = teacher,
    school = school
